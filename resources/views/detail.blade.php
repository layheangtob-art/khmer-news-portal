        ...
        <div class="container">
            ...
            <nav aria-label="breadcrumb" class="mb-4">
                <ol class="breadcrumb mb-0 magnews-breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('news.viewCategory', $news->category->id) }}"
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                    ...
                    <div class="magnews-meta-info mb-4 d-flex align-items-center flex-wrap gap-2">
                        ...
                        @endphp
                        <form action="{{ route('news.like', $news->id) }}" method="POST" class="d-inline m-0 p-0"
                            id="like-form-{{ $news->id }}">
                            @csrf
                            <button type="button"
                                class="btn btn-sm {{ $hasLiked ? 'btn-primary text-white' : 'btn-outline-primary' }} rounded-pill px-3 py-1 d-flex align-items-center like-btn"
                                data-form-id="like-form-{{ $news->id }}"
                                style="font-size: 13px; font-weight: 500; transition: all 0.2s ease;">
                                <i class="{{ $hasLiked ? 'fas' : 'far' }} fa-thumbs-up me-1 like-icon"></i> <span
                                    class="like-count">{{ $news->likes()->count() }}</span>
                            </button>
                        </form>
                    </div>
                                    ...
                                    <div class="col-lg-6">
                                        <div class="magnews-related-post">
                                            ...
                                            <div class="magnews-related-content">
                                                <a href="{{ route('news.show', $relatedNews->id) }}"
                                                    class="magnews-related-title-link">
                                                    {{ Str::limit($relatedNews->title, 60) }}
                                                </a>
                                                <p class="magnews-related-date mb-0">
                                                    {{ $relatedNews->category->name }} -
                                                    {{ $relatedNews->created_at->translatedFormat('M d') }}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                @foreach ($popularPosts as $popularPost)
                                    <div class="magnews-popular-post-item">
                                        ...
                                        <div class="magnews-popular-content">
                                            <a href="{{ route('news.show', $popularPost->id) }}"
                                                class="magnews-popular-title">
                                                {{ Str::limit($popularPost->title, 50) }}
                                            </a>
                                            <p class="magnews-popular-date mb-0">
                                                {{ $popularPost->category->name }} -
                                                {{ $popularPost->created_at->translatedFormat('M d') }}
                                            </p>
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        <div class="magnews-sidebar-widget">
                            ...
                            <div class="magnews-tags-widget">
                                @foreach ($categories->take(10) as $tagCategory)
                                    <a href="{{ route('news.viewCategory', $tagCategory->id) }}"
                                        class="magnews-tag-small">{{ $tagCategory->name }}</a>
                                @endforeach
                            </div>
                        </div>
                    </div>
